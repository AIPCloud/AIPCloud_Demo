...
class SpeechToText(speech_to_text_pb2_grpc.SpeechToTextServicer):
    ...
    def Recognition(self, request, context):
        ...
        try:
            ...
            operation = self.speechClient.long_running_recognize(config=config, audio=audio)
        ...
        return speech_to_text_pb2.Response(transcript=results, exec_time=execTime)

def serve():
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    speech_to_text_pb2_grpc.add_SpeechToTextServicer_to_server(SpeechToText(), server)
